...
class VOCSBDClassification(data.Dataset):
    ...
    def __init__(
        ...
        self.images = [
            os.path.join(image_dir, x + '.jpg')
            for x in voc_file_names
        ]
        self.images += [
            os.path.join(sbd_image_dir, x + '.jpg')
            for x in sbd_file_names
        ]
        self.masks = [
            os.path.join(mask_dir, x + '.png')
            for x in voc_file_names
        ]
        self.masks += [
            os.path.join(sbd_mask_dir, x + '.mat')
            for x in sbd_file_names
        ]
        assert (len(self.images) == len(self.masks))
