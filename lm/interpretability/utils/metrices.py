...
SMOOTH = 1e-6
__all__ = [
    'get_f1_scores', 'get_ap_scores', 'batch_pix_accuracy', 'batch_intersection_union', 'get_iou', 'get_pr',
...
def get_ap_multiclass(predict, target):
    ...
    for pred, tgt in zip(predict, target):
        ...
        total.append(
            np.nan_to_num(
            average_precision_score(target_flat, predict_flat),
            ),
        )
...
def intersection_and_union(im_pred, im_lab, num_class):
    ...
    area_inter, _ = np.histogram(
        intersection, bins=num_class - 1,
        range=(1, num_class - 1),
    )
    # Compute area union:
    area_pred, _ = np.histogram(
        im_pred, bins=num_class - 1,
        range=(1, num_class - 1),
    )
    area_lab, _ = np.histogram(
        im_lab, bins=num_class - 1,
        range=(1, num_class - 1),
    )
    area_union = area_pred + area_lab - area_inter
    return area_inter, area_union
