    ...
    with h5py.File(os.path.join(args.method_dir, 'results.hdf5'), 'a') as f:
        ...
        for batch_idx, (data, target) in enumerate(tqdm(sample_loader)):
            if first:
                ...
                data_image.resize(
                    data_image.shape[0] + data.shape[0] - 1, axis=0,
                )
                data_target.resize(
                    data_target.shape[0] + data.shape[0] - 1, axis=0,
                )
            else:
                data_cam.resize(data_cam.shape[0] + data.shape[0], axis=0)
                data_image.resize(data_image.shape[0] + data.shape[0], axis=0)
                data_target.resize(
                    data_target.shape[0] + data.shape[0], axis=0,
                )
            ...
            if args.method == 'rollout':
                Res = baselines.generate_rollout(
                    data, start_layer=1,
                ...
                Res = lrp.generate_LRP(data, start_layer=1, method='grad', index=index).reshape(
                    data.shape[0], 1, 14, 14,
                )
                # Res = Res - Res.mean()
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        choices=[
            'rollout', 'lrp', 'transformer_attribution', 'full_lrp', 'lrp_last_layer',
    ...
    parser.add_argument(
        '--cls-agn', action='store_true',
        default=False,
        help='',
    )
    parser.add_argument(
        '--no-ia', action='store_true',
        default=False,
        help='',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--no-fgx', action='store_true',
        default=False,
        help='',
    )
    parser.add_argument(
        '--no-m', action='store_true',
        default=False,
        help='',
    )
    parser.add_argument(
        '--no-reg', action='store_true',
        default=False,
        help='',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--imagenet-validation-path', type=str,
        required=True,
        help='',
    )
    ...
    try:
        os.remove(
            os.path.join(
                PATH, 'visualizations/{}/{}/results.hdf5'.format(
                    args.method,
                    args.vis_class,
                ),
            ),
    ...
    os.makedirs(
        os.path.join(
        PATH, 'visualizations/{}'.format(args.method),
        ), exist_ok=True,
    )
