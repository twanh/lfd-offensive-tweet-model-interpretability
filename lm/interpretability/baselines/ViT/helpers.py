    ...
    for n, m in parent_module.named_modules():
        ...
        if isinstance(old_module, nn.BatchNorm2d):
            new_bn = nn.BatchNorm2d(
                num_features=state_dict[
                    n +
                    '.weight'
                ][0], eps=old_module.eps, momentum=old_module.momentum,
...
def build_model_with_cfg(
    ...
    model = model_cls(
        **kwargs,
    ) if model_cfg is None else model_cls(cfg=model_cfg, **kwargs)
