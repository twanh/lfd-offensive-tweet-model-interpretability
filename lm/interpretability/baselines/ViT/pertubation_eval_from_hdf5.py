...
def eval(args):
    ...
    for batch_idx, (data, vis, target) in enumerate(tqdm(sample_loader)):
        ...
        pred_org_prob = pred_probabilities.data.max(1, keepdim=True)[
            0
        ].squeeze(1)
        pred_class = pred.data.max(1, keepdim=True)[1].squeeze(1)
        tgt_pred = (target == pred_class).type(
            target.type(),
        ).data.cpu().numpy()
        ...
        for i in range(len(perturbation_steps)):
            ...
            _, idx = torch.topk(
                vis, int(base_size * perturbation_steps[i]), dim=-1,
            )
            ...
            pred_prob = pred_probabilities.data.max(1, keepdim=True)[
                0
            ].squeeze(1)
            ...
            temp = torch.log(target_probs / second_probs).data.cpu().numpy()
            dissimilarity_pertub[
                i,
                perturb_index:perturb_index+len(temp),
            ] = temp
    ...
    print(
        np.mean(dissimilarity_pertub, axis=1),
        np.std(dissimilarity_pertub, axis=1),
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        choices=[
            'rollout', 'lrp', 'transformer_attribution', 'full_lrp', 'v_gradcam', 'lrp_last_layer',
    ...
    if args.vis_class == 'index':
        args.runs_dir = os.path.join(
            PATH, 'experiments/perturbations/{}/{}_{}'.format(
                exp_name,
                args.vis_class,
                args.class_id,
            ),
        ...
        args.runs_dir = os.path.join(
            PATH, 'experiments/perturbations/{}/{}/{}'.format(
                exp_name,
    ...
    experiments = sorted(
        glob.glob(os.path.join(args.runs_dir, 'experiment_*')),
    )
    experiment_id = int(
        experiments[-1].split('_')
        [-1],
    ) + 1 if experiments else 0
