...
parser.add_argument(
    '--arc', type=str, default='vgg', metavar='N',
    help='Model architecture',
)
parser.add_argument(
    '--train_dataset', type=str, default='imagenet', metavar='N',
...
def eval_batch(image, labels, evaluator, index):
    ...
    if args.save_img:
        ...
        Image.fromarray(img, 'RGB').save(
            os.path.join(
            saver.results_dir, 'input/{}_input.png'.format(index),
            ),
        )
    ...
    # segmentation test for our method
        ...
        Res = lrp.generate_LRP(
            image.cuda(
            ), start_layer=1, method='transformer_attribution',
        ).reshape(batch_size, 1, 14, 14)
...
plt.savefig(
    os.path.join(
        saver.experiment_dir,
        'PR_curve_{}.png'.format(args.method),
    ),
)
