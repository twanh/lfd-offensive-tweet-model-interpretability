...
import numpy as np
import torch
from BERT_rationale_benchmark.utils import Annotation
from BERT_rationale_benchmark.utils import annotations_from_jsonl
from BERT_rationale_benchmark.utils import Evidence
from BERT_rationale_benchmark.utils import load_documents
from BERT_rationale_benchmark.utils import load_flattened_documents
from BERT_rationale_benchmark.utils import load_jsonl
    ...
    def from_instance(cls, inst: dict) -> List['Rationale']:
        ...
        for rat in inst['rationales']:
            for pred in rat.get('hard_rationale_predictions', []):
                ret.append(
                    Rationale(
                    inst['annotation_id'], rat['docid'], pred['start_token'], pred['end_token'],
                    ),
                )
        return ret
        ...
        for inst in instances:
            for rat in inst['rationales']:
                ...
                ret.append(
                    ...
                    inst['annotation_id'], docid, tuple(scores), tuple(key_to_annotation[key]),
                    ),
                )
        return ret
...
def partial_match_score(truth: List[Rationale], pred: List[Rationale], thresholds: List[float]) -> List[Dict[str, Any]]:
    ...
    for threshold in thresholds:
        ...
        micro_r = sum(threshold_tps.values()) / sum(
            num_truth.values(),
        ) if sum(num_truth.values()) > 0 else 0
        micro_p = sum(threshold_tps.values()) / sum(
            num_classifications.values(),
        ) if sum(num_classifications.values()) > 0 else 0
        micro_f1 = _f1(micro_r, micro_p)
        macro_rs = list(
            threshold_tps.get(k, 0.0) / n if n >
            0 else 0 for k, n in num_truth.items()
        )
        macro_ps = list(
            threshold_tps.get(k, 0.0) / n if n >
            0 else 0 for k, n in num_classifications.items()
        )
...
def score_hard_rationale_predictions(truth: List[Rationale], pred: List[Rationale]) -> Dict[str, Dict[str, float]]:
    ...
    for k in set(ann_to_rat.keys()) | (pred_to_rat.keys()):
        if len(pred_to_rat.get(k, set())) > 0:
            instance_prec = len(
                ann_to_rat.get(k, set()) &
                pred_to_rat.get(k, set()),
            ) / len(pred_to_rat[k])
        else:
            instance_prec = 0
    ...
    macro_prec = sum(
        instance['p'] for instance in instances_to_scores.values(
        )
    ) / len(instances_to_scores)
    macro_rec = sum(
        instance['r'] for instance in instances_to_scores.values(
        )
    ) / len(instances_to_scores)
    macro_f1 = sum(
        instance['f1'] for instance in instances_to_scores.values(
        )
    ) / len(instances_to_scores)
...
def score_classifications(instances: List[dict], annotations: List[Annotation], docs: Dict[str, List[str]], aopc_thresholds: List[float]) -> Dict[str, float]:
    ...
    if 'comprehensiveness_classification_scores' in instances[0]:
        comprehensiveness_scores = [
            ...
            x['comprehensiveness_classification_scores'][x['classification']] for x in instances
        ]
    ...
    if 'sufficiency_classification_scores' in instances[0]:
        sufficiency_scores = [
            x['classification_scores'][x['classification']] -
            x['sufficiency_classification_scores'][x['classification']] for x in instances
        ]
    ...
    if 'comprehensiveness_classification_scores' in instances[0]:
        comprehensiveness_entropies = [
            entropy(
                list(
                    x['classification_scores'].values(
                    ),
                ),
            ) - entropy(list(x['comprehensiveness_classification_scores'].values())) for x in instances
        ]
        comprehensiveness_entropy = np.average(comprehensiveness_entropies)
        comprehensiveness_kl = np.average(
            list(
                compute_kl(
                x['classification_scores'], x['comprehensiveness_classification_scores'],
                ) for x in instances
            ),
        )
    else:
    ...
    if 'sufficiency_classification_scores' in instances[0]:
        sufficiency_entropies = [
            ...
            ) - entropy(list(x['sufficiency_classification_scores'].values())) for x in instances
        ]
...
def verify_instances(instances: List[dict], docs: Dict[str, list]):
    ...
    if 'thresholded_scores' in instances[0]:
        thresholds = set(
            x['threshold']
            for x in instances[0]['thresholded_scores']
        )
    else:
        thresholds = None
...
def main():
    parser = argparse.ArgumentParser(
        description="""Computes rationale and final class classification scores""", formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--strict', dest='strict', required=False,
        action='store_true', default=False, help='Do we perform strict scoring?',
    )
    parser.add_argument(
        '--results', dest='results', required=True, help="""Results File
    ...
    parser.add_argument(
        '--iou_thresholds', dest='iou_thresholds', required=False, nargs='+', type=float, default=[0.5], help='''Thresholds for IOU scoring.
    ...
    parser.add_argument(
        '--score_file', dest='score_file',
        required=False, default=None, help='Where to write results?',
    )
    parser.add_argument(
        '--aopc_thresholds', nargs='+', required=False, type=float,
        default=[0.01, 0.05, 0.1, 0.2, 0.5], help='Thresholds for AOPC Thresholds',
    )
    ...
    docids |= set(
        chain.from_iterable(
        (ev.docid for ev in chain.from_iterable(ann.evidences)) for ann in annotations
        ),
    )
    ...
    if _has_hard_predictions(results):
        truth = list(
            chain.from_iterable(
                Rationale.from_annotation(ann)
                for ann in annotations
            ),
        )
        pred = list(
            chain.from_iterable(
                Rationale.from_instance(inst)
                for inst in results
            ),
        )
