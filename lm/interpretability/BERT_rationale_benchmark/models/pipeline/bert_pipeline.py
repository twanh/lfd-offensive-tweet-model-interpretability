...
from BERT_rationale_benchmark.utils import write_jsonl
...
def generate(text_list, attention_list, latex_file, color='red', rescale_value=False):
    ...
    if attention_list.max() == attention_list.min():
        ...
        attention_list = 100 * (attention_list - attention_list.min()) / \
...
def initialize_models(params: dict, batch_first: bool, use_half_precision=False):
    ...
    evidence_classes = dict((y, x) for (x, y) in enumerate(
        params['evidence_classifier']['classes'],
    ))
...
def main():
    ...
    * decode first the evidence, then run classification for each split
    ...
    parser.add_argument(
        '--model_params', dest='model_params', required=True,
    ...
    model_save_file = os.path.join(
        evidence_classifier_output_dir, 'classifier.pt',
    )
    ...
    max_grad_norm = model_params['evidence_classifier'].get(
        'max_grad_norm', None,
    )
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_start in range(0, len(epoch_train_data), batch_size):
            batch_elements = epoch_train_data[
                batch_start:min(
                batch_start + batch_size, len(epoch_train_data),
                )
            ]
            targets = [
                evidence_classes[s.classification]
                for s in batch_elements
            ]
            targets = torch.tensor(targets, dtype=torch.long, device=device)
            samples_encoding = [
                interned_documents[extract_docid_from_dataset_element(s)] for s in batch_elements
            ]
            input_ids = torch.stack([samples_encoding[i]['input_ids'] for i in range(len(samples_encoding))]).squeeze(
                1,
            ).to(device)
            attention_masks = torch.stack(
                [samples_encoding[i]['attention_mask'] for i in range(len(samples_encoding))],
            ).squeeze(1).to(device)
            ...
            epoch_training_acc += accuracy_score(
                preds.argmax(
                dim=1,
                ).cpu(), targets.cpu(), normalize=False,
            )
        ...
        with torch.no_grad():
            ...
            for batch_start in range(0, len(epoch_val_data), val_batch_size):
                batch_elements = epoch_val_data[
                    batch_start:min(
                    batch_start + val_batch_size, len(epoch_val_data),
                    )
                ]
                targets = [
                    evidence_classes[s.classification]
                ...
                input_ids = torch.stack(
                    [samples_encoding[i]['input_ids'] for i in range(len(samples_encoding))],
                ).squeeze(1).to(device)
                attention_masks = torch.stack(
                    [samples_encoding[i]['attention_mask'] for i in range(len(samples_encoding))],
                ).squeeze(1).to(
    ...
    if os.path.exists(epoch_save_file):
        ...
        method_folder = {
            'transformer_attribution': 'ours', 'partial_lrp': 'partial_lrp', 'last_attn': 'last_attn',
        ...
        for i in range(5, 85, 5):
            result_files.append(
                open(
                    ...
                    ).format(method_folder[method], i), 'w',
                ),
            )
        ...
        for batch_start in range(0, len(test), test_batch_size):
            batch_elements = test[
                batch_start:min(
                batch_start + test_batch_size, len(test),
                )
            ]
            targets = [
                evidence_classes[s.classification]
            ...
            for s in batch_elements:
                ...
                is_classification_correct = 1 if preds.argmax(
                    dim=1,
                ) == targets else 0
...
\end{document}
                ...
                if method == 'ground_truth':
                    ...
                    generate(
                        inp_cropped, cam,
                        (
                            os.path.join(args.output_dir, '{0}/visual_results_{1}.tex').format(
                                method_folder[method],
                                j,
                            )
                        ), color='green',
                ...
                cam_target = method_expl[method](
                    input_ids=input_ids, attention_mask=attention_masks, index=target_idx,
                )[0]
                ...
                if method in ['transformer_attribution', 'partial_lrp', 'attn_gradcam', 'lrp']:
                    cam_false_class = method_expl[method](
                        input_ids=input_ids, attention_mask=attention_masks, index=1-target_idx,
                    )[0]
                    cam_false_class = cam_false_class.clamp(min=0)
                    generate(
                        text, cam_false_class,
                        (
