...
from rationale_benchmark.metrics import partial_match_score
from rationale_benchmark.metrics import PositionScoredDocument
from rationale_benchmark.metrics import Rationale
from rationale_benchmark.metrics import score_hard_rationale_predictions
from rationale_benchmark.metrics import score_soft_tokens
    ...
    for ann in annotations:
        ...
        for docid, evidences in docid_to_ev.items():
            ...
            for evidence in evidences:
                ...
                assert evidence.start_token < len(
                    doc,
                ) and evidence.end_token <= len(doc)
                text.extend(evidence.text)
                new_tokens = set(
                    range(evidence.start_token, evidence.end_token),
                )
...
def annotations_to_evidence_token_identification(
    ...
    for ann in annotations:
        ...
        for docid in docids:
            start = 0
            assert len(source_documents[docid]) == len(
                interned_documents[docid],
            )
...
def make_preds_epoch(
    ...
    for batch in batches:
        loss, soft_preds, hard_preds, targets = make_preds_batch(
            classifier, batch, device, criterion=criterion,
...
def make_token_preds_batch(
    ...
    if criterion:
        ...
        loss = None
    hard_preds = [
        torch.round(x).to(dtype=torch.int).cpu()
        for x in targets.unpad(preds)
    ]
    targets = [[y.item() for y in x]
               for x in targets.unpad(targets.data.cpu())]
    # targets.unpad(targets.data.cpu())
    return loss, preds, hard_preds, targets
...
def score_rationales(
    ...
    for sent, score in zip(input_data, scores):
        ...
        if k not in doc_to_sent_scores:
            doc_to_sent_scores[k] = [
                0.0 for _ in range(len(documents[sent.docid]))
            ]
    ...
    best_sentence = {
        k: np.argmax(np.array(v))
        for k, v in doc_to_sent_scores.items()
    }
    ...
    for (ann_id, docid), sent_idx in best_sentence.items():
        ...
        predicted_rationales.append(
            Rationale(ann_id, docid, start_token, end_token),
        )
    true_rationales = list(
        chain.from_iterable(
        Rationale.from_annotation(rat) for rat in truth
        ),
    )
    ...
    for (ann_id, docid), sentences in doc_to_sent_scores.items():
        ...
        for sent_score, sent_text in zip(sentences, documents[docid]):
            soft_token_predictions.extend(
                sent_score for _ in range(len(sent_text))
            )
    ...
    flattened_documents = {
        k: list(chain.from_iterable(v))
        for k, v in documents.items()
    }
    token_scoring_format = PositionScoredDocument.from_results(
        instance_format, truth, flattened_documents,
        use_tokens=True,
    )
    ...
    results['soft_sentence_scores'] = score_soft_tokens(
        sentence_scoring_format,
    )
    return results
...
def decode(
    ...
    class_labels = [
        k for k, v in sorted(
        class_interner.items(), key=lambda x: x[1],
        )
    ]
    ...
    def prep(data: List[Annotation]) -> List[Tuple[SentenceEvidence, SentenceEvidence]]:
        ...
        """
        identification_data = annotations_to_evidence_identification(
            data, docs,
        )
        ...
        ann_doc_sents = defaultdict(lambda: defaultdict(dict))
        ret = []
    ...
    def decode_batch(
        data: List[Tuple[SentenceEvidence, SentenceEvidence]], name: str, score: bool = False,
        ...
        with torch.no_grad():
            ...
            _, soft_identification_preds, hard_identification_preds, _ = make_preds_epoch(
                evidence_identifier,
            ...
            for id_data, cls_data, soft_id_pred, hard_id_pred in zip(
                identifier_data, classifier_data,
                ...
                res = IdentificationClassificationResult(
                    ...
                    soft_identification=soft_id_pred[1].float(
                    ).item(),
                ...
                identification_results[(
                    id_data.ann_id, id_data.docid,
                )].append(res)

            best_identification_results = {
                key: max(value, key=lambda x: x.soft_identification) for key, value in
            ...
            if score:
                ...
                results[f'{name}_rationale'] = score_rationales(
                    annotations, decoding_docs, identifier_data,
                    soft_identification_preds,
                )
            ...
            for (ann_id, docid), pred in classification_results.items():
                sentence_prediction_scores = [
                    x.soft_identification for x in identification_results[(ann_id, docid)]
                ]
                sentence_start_token = sum(
                    len(s) for s in decoding_docs[docid][:pred.identification_data.index]
                )
                sentence_end_token = sentence_start_token + \
                    len(decoding_docs[docid][pred.classification_data.index])
                hard_rationale_predictions = [
                    {'start_token': sentence_start_token, 'end_token': sentence_end_token},
                ]
                ...
                for sent_result in sorted(
                    ...
                    soft_rationale_predictions.extend(
                        sent_result.soft_identification for _ in range(
                            len(
                ...
                if ann_id not in decoded:
                    decoded[ann_id] = {
                        ...
                        'classification_scores': {
                            class_labels[i]: s.item() for i, s in
...
def decode_evidence_tokens_and_classify(
    ...
    def decode_batch(
        data: List[Tuple[SentenceEvidence, SentenceEvidence]], name: str, score: bool = False,
        annotations: List[Annotation] = None, class_labels: dict = class_labels,
    ) -> dict:
        """Identifies evidence statements and then makes classifications based on it.
        ...
        with torch.no_grad():
            ...
            if use_cose_hack:
                ...
                for i in range(0, len(soft_identification_preds), grouping):
                    ...
                    cls_labels = [
                        x.ann_id.split(
                        '_',
                        )[-1] for x in evidence_only_cls[i:i + grouping]
                    ]
                    # we need to update the class labels because of the terrible hackery used to train this
                    class_labels = cls_labels
                    cls_truths = [
                        x.kls for x in evidence_only_cls[i:i + grouping]
                    ]
            ...
            if score:
                ...
                soft_id_scores = [[1-x, x]
                                  for x in chain.from_iterable(soft_identification_preds)]
                results[f'{name}_rationale'] = score_rationales(
                    annotations,
                    decoding_docs,
                    identifier_data,
            ...
            for id_data, soft_id_pred, hard_id_pred, soft_cls_preds, hard_cls_pred in zip(
                identifier_data,
                soft_identification_preds,
                hard_identification_preds,
                ...
                for pos, (start, _) in enumerate(token_mapping[id_data.docid][0]):
                    rationales['soft_rationale_predictions'].append(
                        soft_id_pred[start],
                    )
                    if bool(hard_id_pred[start]):
                        if start_span == -1:
