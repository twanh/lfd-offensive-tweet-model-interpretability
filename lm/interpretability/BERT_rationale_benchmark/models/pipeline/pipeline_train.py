...
from rationale_benchmark.models.mlp import BahadanauAttention
from rationale_benchmark.models.mlp import RNNEncoder
from rationale_benchmark.models.mlp import WordEmbedder
...
from rationale_benchmark.utils import intern_annotations
from rationale_benchmark.utils import intern_documents
from rationale_benchmark.utils import load_datasets
from rationale_benchmark.utils import load_documents
from rationale_benchmark.utils import write_jsonl
...
def initialize_models(params: dict, vocab: Set[str], batch_first: bool, unk_token='UNK'):
    ...
    document_encoder = RNNEncoder(
        ...
        attention_mechanism=BahadanauAttention(
            word_embedder.output_dimension,
...
def main():
    ...
    * decode first the evidence, then run classification for each split
    ...
    docids = set(
        e.docid for e in chain.from_iterable(
            chain.from_iterable(
    ...
    document_vocab = set(
        chain.from_iterable(
        chain.from_iterable(documents.values()),
        ),
    )
    annotation_vocab = set(
        chain.from_iterable(
            e.query.split()
            for e in chain(train, val, test)
        ),
    )
    logging.debug(
    ...
    evidence_identifier, evidence_classifier, word_interner, de_interner, evidence_classes = \
        initialize_models(
            model_params, vocab,
            batch_first=BATCH_FIRST, unk_token=unk_token,
        )
    logging.debug(
    ...
    write_jsonl(
        train_decoded, os.path.join(
        args.output_dir, 'train_decoded.jsonl',
        ),
    )
    write_jsonl(
        val_decoded, os.path.join(
        args.output_dir, 'val_decoded.jsonl',
        ),
    )
    write_jsonl(
        test_decoded, os.path.join(
        args.output_dir, 'test_decoded.jsonl',
        ),
    )
